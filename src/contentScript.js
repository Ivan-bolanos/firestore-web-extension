...
function extractDocumentData() {
    try {
        ...
        if (!documentUrl.includes('/firestore/data/') && !documentUrl.includes('/firestore/databases/')) {
            console.log('Not on a Firestore document page');
            return { url: documentUrl, data: null, error: 'Not on a Firestore document page' };
        }
        ...
        if (dataTrees.length === 0) {
            return {
                url: documentUrl,
                data: null,
                error: 'No document fields found. Make sure you are viewing a document.'
            };
        }
        ...
        for (const tree of dataTrees) {
            ...
            if (tree.closest('.database-children') && tree.parentElement.closest('f7e-data-tree')) {
                continue;
            }
...
function parseDataTree(treeElement, targetObject) {
    ...
    const type = typeElement ? typeElement.textContent.trim().replace(/[()]/g, '') : '';
    ...
    const childrenContainer = treeElement.querySelector(':scope > .database-node > .database-children');

    if (childrenContainer && childrenContainer.children.length > 0) {
        // It's a nested structure (object or array)
        const childTrees = childrenContainer.querySelectorAll(':scope > f7e-data-tree');
